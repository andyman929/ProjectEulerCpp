...
void ProjectsObj::Project11Calc(int line)
{
	...
	for (int row = 0; row < 20; row++)
	{
		for (int col = 0; col < 20; col++)
		{
			...
			if (col <= 20 - line)
			{
				...
				for (int i = 0; i < line; i++)
				{
					product *= DataGrid[col + i][row];
				}
				if (product > maxProduct)
					maxProduct = product;
			}
			// diag right
			if (row <= 20 - line && col <= 20 - line)
			{
				product = 1;
				for (int i = 0; i < line; i++)
				{
					product *= DataGrid[col + i][row + i];
				}
				if (product > maxProduct)
					maxProduct = product;
			}
			// diag left
			if (row <= 20 - line && col >= line - 1)
			{
				product = 1;
				for (int i = 0; i < line; i++)
				{
    ...
    cout << "Maximum product for a line of " << line << " is: " << maxProduct;
}
